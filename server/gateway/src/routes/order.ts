...
import {RESPONSE_STATUS, CHECK_IF_EMPTY, GENERATE_TOKEN} from '../../../engine/src/include/index';
...
module.exports = (router:any) => {  
    router.all("/order/:api_path?", VALIDATE_SESSION, (req: Request, res: Response) => {
        ...
        .then(() => {
            ...
            let path:string = LOAD_BALANCE("order", "golang", i)+params.replace("/?", "?");
            let session_key:string = user.user_id+"-"+LOAD_BALANCE("order", "golang", i)+api_path; // generate request session key 
            ...
            {  
                // if(x > 1)
                // { //check if request is retried, then create a new request session key
                //     let session_key:string = decrypt_session.user_id+req.baseUrl + req.path+"/request"; // generate request session key
                //     GENERATE_REQUEST_SESSION(session_key);
                // }
                ...
                .catch((err:any) => { 
                    ...
                    let check_timeout:boolean = _timeout.includes("timeout"); // check if it was cause of request timeout

                    if(check_timeout && x <= 3) RETRY_CALL(x+1); // reinitiate call if it was timeout
